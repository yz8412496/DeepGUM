...
from data_generator_shared import load_data_generator,load_data_generator_simple
from data_generator_shared import load_data_generator_List, load_data_generator_Uniform_List,rnEqui,rnHard,rnTra
...
class MixtureModel:
    ...
    def fit(self, ROOTPATH, trainT, valT, test_txt,learning_rate=0.1, itmax=2,validation=validationRatio,subsampling=1.0):
        ...
        self.network.compile(optimizer=rms, loss={'classification': 'categorical_crossentropy', 'regression': 'mse'},
    ...
    def M_step_network(self, ROOTPATH, trainT, valT, test_txt, learning_rate, nbEpoch=NB_EPOCH):
        ...
        self.networkRn.compile(optimizer=rms, loss={'classification': 'categorical_crossentropy', 'merge': 'mse'},
            metrics={'classification': 'accuracy', 'merge': 'mea£ú<ı.>c¯–Ñ>#¯¦Å>,ï¢3Æj-âsöZ9í<²n
...
LøÊ=hîQsƒš÷°¹KÍªy@İæ{õ¸öy:],Æb-bq[±‹¼˜[°Ä‹ì˜²JpÈÛéd±‹°˜‹ÕX ÅGìÍr*Z`Ä×ì¡rš}TŞ@§Å[ìÄ²,ŠbÖua×µaH×‰aY…1\ë†0+Ö`¡Ç±mK’H’IRII>I/‰#Ù&%%$ää´´ˆˆ™YU üÿş?ÿ¯ÿÃÿî?ó¯úüş>?¯¯ÃÃî.3£ª ı?şoÿ“ÿÒ?âoöSù=>no““Ò"rfZUÀü¯şÿ¾?Ï¯ëÃğ®;Ã¬®¾~Ÿ»×Ì¡jıSşB?oÛ“ä’4’h’QRC‚NK·ˆ‰YY<üî>3¯ªÀş/ÿ£ÿÆ?í/òcúV<Á./££Æ-="nfS•şsÿš?Ôïà³÷Ê9hírsšZÄğ¬»Â®zœş?±/Ë£è†1]+†`Öqa—´‘H“‰Ru>Xï…sÜÚ&$å$´äˆ´™H•	Pùı>>o¯“ÃÒ."c¦V<ÿ®?Ã¯îó¾:¬ûÂ<®n“¾²{Ê\¨Æm?’oÒSâB6Ni‘8“­RB~N_‹‡ØeVTÁ ¯¿ÃÏî+ó ºÌıj>PïƒóŞ:'¬åB4Îh«‘@“Òât¶X‰Y<Å.,ã¢6i=.s£šı0¾kÏ«ÓÀ¢/Æcí2q*[ Ä‡ìrZqÛ¼¤›¼”›³ÔŠ ˜çÕu`Ø×åat×˜¡