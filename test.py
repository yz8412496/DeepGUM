...
def run_eval(Y_pred, Y_true, l, pbFlag, C_pred=[], C_true=[], idOar="", printError=False):
    # print "Evaluating"
    ...
    if (pbFlag == 'landmark'):
        ...
        for j in range(Y_pred.shape[0]):
            if (Y_pred[j] == Y_true[j]):
                count+=1
        print ("Accuracy:" + str(count/Y_pred.shape[0]))

    elif (pbFlag == 'Joint' or pbFlag == 'Shared'):
        count = 0
        for j in range(C_pred.shape[0]):
            if (C_pred[j] == C_true[j]):
                count+=1
        print ("Accuracy:" + str(count/C_pred.shape[0]))
