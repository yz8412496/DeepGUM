...
def load_data_generator_List(rootpath, imIn, file_test, validation=1.0,subsampling=1.0,processingTarget=None,transform=[],outSize=(224,224),batch_size=BATCH_SIZE,shuffle=False):
    ...
    def generator(rootpath, images):
        ...
        while 1:
            ...
            yield(X, [Z, Y[:,0], Y[:,1], Y[:,2], Y[:,3], Y[:,4], Y[:,5], Y[:,6], Y[:,7], Y[:,8], Y[:,9], Y[:,10], Y[:,11], Y[:,12], 
            Y[:,13], Y[:,14], Y[:,15], Y[:,16], Y[:,17], Y[:,18], Y[:,19], Y[:,20], Y[:,21], Y[:,22], Y[:,23], Y[:,24], Y[:,25], 
...
def load_data_generator_Uniform_List(rootpath, imIn, imVal, file_test,rni, valMode=rnEqui,validation=1.0,subsampling=1.0,outSize=(224,224),processingTarget=None,batch_size=BATCH_SIZE):
    ...
    def generator(rootpath, images,rniIn, batch_size=BATCH_SIZE):
        ...
        while 1:
            ...
            # rni = np.squeeze(rni)
            yield([X, rni[i*batch_size:(i+1)*batch_size]], [Z, Y[:,0], Y[:,1], Y[:,2], Y[:,3], Y[:,4], Y[:,5], Y[:,6], Y[:,7], Y[:,8], Y[:,9], Y[:,10], Y[:,11], Y[:,12], 
            Y[:,13], Y[:,14], Y[:,15], Y[:,16], Y[:,17], Y[:,18], Y[:,19], Y[:,20], Y[:,21], Y[:,22], Y[:,23], Y[:,24], Y[:,25], 
            Y[:,26], Y[:,27], Y[:,28], Y[:,29], Y[:,30], Y[:,31], Y[:,32], Y[:,33], Y[:,34], Y[:,35], Y[:,36], Y[:,37], Y[:,38], 
            Y[:,39], Y[:,40], Y[:,41], Y[:,42], Y[:,43], Y[:,44], Y[:,45], Y[:,46], Y[:,47]])
            # yield(X, Y)
...
def load_data_generator_Uniform(rootpath, file_train, file_val, file_test,rni, valMode=rnEqui, validation=1.0,subsampling=1.0,outSize=(224,224),processingTarget=None,batch_size=BATCH_SIZE):
...
def get_xyz_from_file(rootpath, images, processingTarget=None,transform=[],outSize=(224,224),batch_size=BATCH_SIZE):
    '''Extract data arrays from text file'''
    ...
    # Discretize age labels
    Y = Y - 14
    # Y = tf.keras.utils.to_categorical(Y, num_classes=49).astype(np.float32)
