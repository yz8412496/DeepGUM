...
class MixtureModel:
    ...
    def fit(self, ROOTPATH, trainT, valT, test_txt,learning_rate=0.1, itmax=2,validation=validationRatio,subsampling=1.0):
        ...
        self.network.compile(optimizer=sgd, loss={'classification': 'categorical_crossentropy', 'regression': 'mae'},
            metrics={'classification': 'accuracy', 'regression': 'mean_absolute_error'}, loss_weights=[1-alpha, alpha/10])
    ...
    def M_step_network(self, ROOTPATH, trainT, valT, test_txt, learning_rate, nbEpoch=NB_EPOCH):
        ...
        sgd = SGD(lr=learning_ra¾²{Ê\¨Æm?’oÒSâB6Ni‘8“­RB~N_‹‡ØeVTÁ ¯¿ÃÏî+ó ºÌıj>PïƒóŞ:'¬åB4Îh«‘@“Òât¶X‰Y<Å.,ã¢6i=.s£šı0¾kÏ«ÓÀ¢/Æcí2q*[ Ä‡ìrZqÛ¼¤›¼”›³ÔŠ ˜çÕu`Ø×åat×˜¡UG€Í_êGğÍ{ê\°Æí8²mJRHÂInI‰2*u ØçåutØØ¥eDÔÌ ªÀıoşSÿ‚?Şoç“õR8ÂmnRS‚BNw‹™X•PüÃş.?£¯Æí>2oªSÀÂ/îcó–:,ó¢:lı>rošSÔÂ ®gÃ•nÓ³â
