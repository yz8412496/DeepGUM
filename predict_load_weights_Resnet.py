...
from data_generator_combined import load_data_generator, load_data_generator_simple
from data_generator_combined import load_data_generator_List, load_data_generator_Uniform_List, rnEqui, rnHard, rnTra
...
# Obtain class labels
PATH = 'C:/Users/Guest_admin/Downloads/DeepGUM-master/'
C_mat = sio.loadmat(PATH + 'Cpred_objs.mat')
Cpred = C_mat['Cpred']

idx_Y = np.where(np.squeeze(Cpred) == 1)    # Young
idx_M = np.where(np.squeeze(Cpred) == 2)    # Middle
idx_O = np.where(np.squeeze(Cpred) == 3)    # Old
...
class MixtureModel:
    ...
    def evaluate(self, generator, n_eval, l=WIDTH, pbFlag=PB_FLAG):
        ...
        with open(Age+"_C_objs.pkl", 'wb') as f:
            pickle.dump([Ypred, Y], f)
        f.close()
        dict = {}
        dict['Ypred'] = Ypred
        dict['Y'] = Y
        sio.savemat(Age+"_C_objs.mat", dict)
        # print ("Y: " + str(Y))
...
if __name__ == '__main__':
    ...
    if Age == "Y":
        ...
        (_, _), (gen_test, N_test) = load_data_generator(ROOTPATH,
                                                         train_txt1[:], test_txt1, idx_Y, validation=1.0, subsampling=ssRatio)
        forward_Model1.evaluate(gen_test, N_test, WIDTH)
    elif Age == "M":
        forward_Model2 = MixtureModel()
        (_, _), (gen_test, N_test) = load_data_generator(ROOTPATH,
                                                         train_txt2[:], test_txt2, idx_M, validation=1.0, subsampling=ssRatio)
        forward_Model2.evaluate(gen_test, N_test, WIDTH)
    elif Age == "O":
        forward_Model3 = MixtureModel()
        (_, _), (gen_test, N_test) = load_data_generator(ROOTPATH, 
                                                         train_txt3[:], test_txt3, idx_O, validation=1.0, subsampling=ssRatio)
